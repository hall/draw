...
exports.initPaint = function (svgId) {
  ...
  document.querySelectorAll(".svg-pen vscode-button, .svg-shape vscode-button").forEach(item => {
    click(item, e => {
  ...
  click("#svg-undo", () => {
    ...
    let element = undoList.pop();
    element.remove();
    redoList.push(element);
    redoBoxSizeList.push(boxSizeList.pop());
  });

  click("#svg-redo", () => {
    if (redoList.length < 1) return;
    let element = redoList.pop();
    svg.append(element);
